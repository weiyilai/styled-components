...
- React Native: `createTheme()` works the same way it does on the web. Pass the returned object to `ThemeProvider`, reference leaves in your styled components, and the current theme resolves automatically. ([createTheme-native-parity.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/createTheme-native-parity.md))
...
- A styled component does not honor `defaultProps`, and it does not inherit a wrapped component's `defaultProps`. React 19 removed `defaultProps` support from function components. ([drop-default-props.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/drop-default-props.md))
...
- Mounting the same `createGlobalStyle` component multiple times emits its CSS only once. Rendering output stays byte-stable across SSR, client, and Server Components. ([global-style-useid.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/global-style-useid.md))
- React Native: `isolation: isolate` is supported on iOS, Android, and the web. Blended descendants (`mix-blend-mode`) composite against the isolated element's own group, matching across all three targets. Separately, when a `filter` on iOS uses effects the platform only renders at the experimental release level, development builds warn that the effect is invisible and that it suspends descendant `mix-blend-mode` compositing while mounted. ([isolation-isolate-android-layer.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/isolation-isolate-android-layer.md))
- React Native: CSS-to-style-object translation is built in. The native path supports a broad set of modern CSS: ([native-transform-layer.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-transform-layer.md))
...
- Raised peer dependency floors: ([peer-floors.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/peer-floors.md))
...
- Plugins moved to a dedicated `styled-components/plugins` subpath, and first-party plugins ship there. ([plugins-subpath.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/plugins-subpath.md))
...
- Client styles inject through `useInsertionEffect`. ([buffered-injection-default.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/buffered-injection-default.md))
...
- `createGlobalStyle` rules land before any consumer layout effect. ([global-styles-before-layout-effects.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/global-styles-before-layout-effects.md))
...
- Smaller bundles: `styled.div` and every other element shorthand is built the first time it is read, so an app ships no table of element names and pays only for the tags it uses. ([lazy-element-shorthands.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/lazy-element-shorthands.md))
...
- React Native: modern CSS functions are supported in styles. Values that can be calculated immediately are converted before they reach React Native, while values that depend on the device, like viewport size, safe area, container size, or color scheme, update when those inputs change. ([modern-css-polyfills.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/modern-css-polyfills.md))
...
- React Native: the CSS `attr()` function is supported. Styles can read the component's own props as typed CSS values: `width: attr(data-size px, 48px)` sizes from a `data-size` prop and uses the fallback when the prop is missing or invalid. Supported types include unit names (`px`, `%`, `deg`, ...), `number`, `raw-string`, and `type()` forms for lengths, numbers, percentages, and colors. `attr()` composes inside `calc()`, and fallbacks can themselves be dynamic values like `light-dark()`. ([native-attr-function.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-attr-function.md))
- React Native supports `:not(<simple-selector>)`. Rules such as `:not(:hover)`, `:not(:focus)`, `:not([disabled])`, and `:not([data-state='loading'])` apply when the inner condition does not match. More complex forms, including multiple selectors or nested descendant selectors, are not supported on native: they show a development warning and are ignored. ([native-attr-operators-and-not.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-attr-operators-and-not.md))
- React Native: attribute selectors apply styles based on the rendered element's props. ([native-attribute-selectors.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-attribute-selectors.md))
...
- React Native: `box-sizing` and `hyphens` are supported. ([native-box-sizing-and-hyphens.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-box-sizing-and-hyphens.md))
...
- React Native: several CSS properties are supported across platforms: `caret-color`, `object-fit`, `backface-visibility`, and `outline-offset`. ([native-caret-color-and-passthroughs.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-caret-color-and-passthroughs.md))
...
- React Native: descendant and child combinator selectors are supported across styled components. ([native-combinator-selectors.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-combinator-selectors.md))
...
- React Native: the `corner-shape` property is supported, at the fidelity each value can actually be drawn. ([native-corner-shape.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-corner-shape.md))
...
- React Native supports CSS custom properties through the component cascade. ([native-custom-properties.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-custom-properties.md))
...
- React Native: `field-sizing: content` makes a `TextInput` autosize to its content. ([native-field-sizing-content.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-field-sizing-content.md))
...
- React Native: CSS font properties accept the full CSS grammar and produce identical results on iOS, Android, and the web. ([native-fonts.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-fonts.md))
...
- React Native supports the `:has(<simple>)` selector. ([native-has-selector.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-has-selector.md))
...
- React Native supports CSS `!important`. ([native-important-support.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-important-support.md))
...
- React Native: CSS `overscroll-behavior` and `scrollbar-width` are supported. Apply them to a `styled.ScrollView`, `styled.FlatList`, `styled.SectionList`, or `styled.VirtualizedList`: ([native-scroll-surfaces.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-scroll-surfaces.md))
...
- React Native: styled scrollers (`styled.ScrollView`, `styled.FlatList`, and friends) nest like the web. A vertical scroller inside another vertical scrollable receives gestures on Android without passing `nestedScrollEnabled` yourself; iOS and the web behave this way too. Passing the prop explicitly still wins. ([native-scroller-ergonomics.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-scroller-ergonomics.md))
...
- React Native: sibling combinator selectors and the `:nth-child` family are supported. ([native-sibling-and-nth-child.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-sibling-and-nth-child.md))
...
- Multi-shadow `text-shadow` lists are supported. React Native renders a single shadow per Text element, so on iOS and Android the first (topmost) shadow in the list is applied and a development warning suggests stacking duplicate Text elements for more layers. On the web the browser renders the full list as written. ([native-text-shadow-multiple.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-text-shadow-multiple.md))
- React Native: the CSS tree-counting functions `sibling-index()` and `sibling-count()` are supported. Styles can size, tint, or position each child by where it sits among its siblings, with one shared ruleset: `width: calc(sibling-index() * 10px)` renders a staircase, `width: calc(100% / sibling-count())` divides a row evenly. Values update automatically when siblings mount, unmount, or reorder. ([native-tree-counting-functions.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/native-tree-counting-functions.md))
...
- styled-components does no work in a passive or layout effect on its render path. ([no-passive-or-layout-effects.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/no-passive-or-layout-effects.md))
...
- A `css\`\`\``fragment placed after a declaration that is missing its trailing`;` is treated as a sibling block: ([fragment-missing-semicolon-recovery.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/fragment-missing-semicolon-recovery.md))
...
- Nested rules resolve correctly when their parent selector contains a comma inside `:is()`, `:where()`, `:has()`, or an attribute selector: ([is-where-cross-product-bug.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/is-where-cross-product-bug.md))
...
- Fixed a styling leak between sibling subtrees when a nested `<StyleSheetManager>` sits beside other children of an outer `<StyleSheetManager>` in a server component tree. ([rsc-nested-stylesheetmanager.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/rsc-nested-stylesheetmanager.md))
...
- A React Native scroller only snaps if it declared `scroll-snap-type` itself. ([scroll-snap-needs-a-declared-type.md](https://github.com/weiyilai/styled-components/commits/main/.changeset/scroll-snap-needs-a-declared-type.md))
